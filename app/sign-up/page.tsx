...
export default function SignUpPage() {
  ...
  const handleSignup=()=>{
    ...
    setLoading(true)
    setTimeout(()=>{toast.success('Cuenta creada. Bienvenido a Península');router.push('/')},1000)
  }
  return (
    <div className="page-bare" style={{background:'var(--bg)',minHeight:'100dvh',
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.5}}>
          <Logo size={24}/>
          <p style={{fontSize:13,color:'var(--txt3)',marginTop:8,marginBottom:32}}>
            Crea tu cuenta y recibe agua sin esfuerzo
          </p>
          <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:20}}>
            {[{k:'nombre',ph:'Nombre completo',t:'text'},
              {k:'tel',ph:'Teléfono',t:'tel'},
              {k:'email',ph:'Email',t:'email'},
              {k:'pass',ph:'Contraseña (mín. 4)',t:'password'},
            ].map(f=>(
          ...
          <p style={{textAlign:'center',fontSize:13,color:'var(--txt3)',marginTop:16}}>
            ¿Ya tienes cuenta?{' '}
            <Link href="/sign-in" style={{color:'var(--blue)',fontWeight:600,textDecoration:'none'}}>
              Ingresar
            </Link>
          </p>
        </motion.div>
